...
class ProductHistoryModel(QSqlTableModel):
    ...
    def setProduct(self, asin):
        q = QSqlQuery('SELECT Asin, Timestamp, SalesRank, Offers, Prime, Price FROM Observations WHERE Asin=\'{}\' UNION '
                      'SELECT Asin, Timestamp, SalesRank, Offers, Prime, Price FROM Products WHERE Asin=\'{}\''.format(asin, asin))
